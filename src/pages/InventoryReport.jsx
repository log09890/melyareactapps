...
function SalesRateReport() {
    ...
    }, [rawData, revenue, profit, discountRateInitial]);
    ...
    }, [processedData, filters, searchTerm, month, channel, productName, productId]);
                ...
                <section className="p-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
                    ...
                    <div className="mt-6">
                        {activeTab === 'analysis' && profitMarginChartData && (
                            <div className="h-96"><Bar data={profitMarginChartData} options={{...getCommonChartOptions(theme === 'dark'), indexAxis: 'y', scales: { x: { ticks: { callback: v => `${(v * 100).toFixed(0)}%` }}}, plugins: { legend: { display: false }}}} /></div>
                        )}
                        {activeTab === 'details' && (
                             <div>
